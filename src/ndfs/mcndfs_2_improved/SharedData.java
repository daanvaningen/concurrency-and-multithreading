...
public class SharedData {
  ...
  private volatile Map<State, AtomicInteger> count = new HashMap<State, AtomicInteger>();
  ...
  public void changeCount (State state, int amount) {
    this.count.get(state).addAndAdd(amount);
  }
  ...
  public Integer getCount (State state) {
      currentCount = this.count.get(state).get();
      return currentCount;
  }
}
